...
const LS = { tpl:"tl_tpl_v2", owners:"tl_owners_v2" };
...
const fmtDate = (d, short=false) => {
  ...
  return d.toLocaleDateString("en-GB", short?{day:"numeric",month:"short"}:{day:"numeric",month:"short",year:"numeric"});
};
...
const PAD_LEFT=120, PAD_TOP=52, SUB_H=36, LANE_PAD=12, DAY_W=22, RESIZE_HIT=8;
...
export default function App() {
  ...
  const SVG_H=useMemo(()=>{
    let h=PAD_TOP; lanes.forEach(l=>{h+=LANE_PAD+(laneH[l]??1)*SUB_H+LANE_PAD;}); return h+32;
  },[lanes,laneH]);

  const xOf=d=>PAD_LEFT+d*DAY_W;
  const yOfTask=t=>laneOffsets[t.lane]+LANE_PAD+(subRows[t.id]??0)*SUB_H+4;
  const dateLabel=useCallback((d,short=true)=>fmtDate(addDays(startDate,d),short),[startDate]);
  ...
  const shareURL=()=>{
    ...
    navigator.clipboard.writeText(url).then(()=>{setCopyMsg("Link copied!");setTimeout(()=>setCopyMsg(""),2500);});
  };
  ...
  const exportPDF=()=>{
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>${projectName}</title>
      ...
      <body><h1>${projectName}</h1><p>Start: ${fmtDate(new Date(startDate))} · Generated: ${fmtDate(new Date())}</p>${xml}</body></html>`);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f8f7f4",fontFamily:"'DM Sans',system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      {!presentMode&&(
        <div style={{background:"#fff",borderBottom:"0.5px solid rgba(0,0,0,0.1)",padding:"0 16px",
          height:50,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,gap:8}}>
          <div style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0}}>
            <input value={projectName} onChange={e=>setProjectName(e.target.value)}
              style={{fontSize:14,fontWeight:500,border:"none",outline:"none",background:"transparent",color:"#1a1a1a",minWidth:160,maxWidth:220}}/>
            <div style={{display:"flex",alignItems:"center",gap:5,flexShrink:0}}>
              <label style={{fontSize:11,color:"#aaa"}}>Start:</label>
              <input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)}
                style={{fontSize:12,border:"0.5px solid rgba(0,0,0,0.12)",borderRadius:6,padding:"3px 6px",fontFamily:"inherit",outline:"none"}}/>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
              <label style={{fontSize:11,color:"#aaa"}}>Days:</label>
              <input type="number" value={totalDays} min={7} max={365}
                onChange={e=>setTotalDays(Math.max(7,parseInt(e.target.value)||28))}
                style={{width:50,fontSize:12,padding:"3px 6px",border:"0.5px solid rgba(0,0,0,0.12)",borderRadius:6,fontFamily:"inherit",outline:"none"}}/>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:2,background:"#f1efe8",borderRadius:20,padding:"2px 3px",flexShrink:0}}>
              {["lane","owner","tag","rag"].map(m=>(
                <button key={m} onClick={()=>setColourMode(m)} style={{
                  fontSize:11,padding:"3px 8px",borderRadius:16,border:"none",cursor:"pointer",fontFamily:"inherit",
                  background:colourMode===m?"#1a1a1a":"transparent",color:colourMode===m?"#fff":"#666"}}>
                  By {m==="rag"?"RAG":m}
                </button>
              ))}
            </div>
          ...
          <div style={{display:"flex",gap:5,flexShrink:0,alignItems:"center"}}>
            {copyMsg&&<span style={{fontSize:11,color:"#5DCAA5"}}>{copyMsg}</span>}
            ...
            <Btn onClick={()=>setModal("export")} accent>Export ▾</Btn>
            <Btn onClick={()=>setPresentMode(true)}>Present</Btn>
          </div>
        </div>
      )}

      {presentMode&&(
        <div style={{background:"#1a1a1a",padding:"8px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <span style={{color:"#fff",fontSize:14,fontWeight:500}}>{projectName}</span>
          <div style={{display:"flex",gap:16,alignItems:"center"}}>
            <span style={{color:"#888",fontSize:12}}>Start: {fmtDate(new Date(startDate))}</span>
            <button onClick={()=>setPresentMode(false)}
              style={{fontSize:12,padding:"4px 12px",border:"0.5px solid #555",borderRadius:6,background:"transparent",color:"#ccc",cursor:"pointer",fontFamily:"inherit"}}>Exit</button>
          </div>
        </div>
      )}

      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        <div style={{flex:1,overflow:"auto",padding:presentMode?12:20}}>
          <div style={{background:"#fff",borderRadius:12,border:"0.5px solid rgba(0,0,0,0.08)",display:"inline-block",minWidth:"100%"}}>
            <svg ref={svgRef} width={SVG_W} height={SVG_H} style={{display:"block",userSelect:"none"}} xmlns="http://www.w3.org/2000/svg">
              <defs>
              ...
              {Array.from({length:Math.ceil(totalDays/7)+1},(_,i)=>i*7).filter(d=>d<=totalDays).map(d=>(
                <g key={d}>
                  <line x1={xOf(d)} y1={PAD_TOP-16} x2={xOf(d)} y2={SVG_H-24} stroke="rgba(0,0,0,0.07)" strokeWidth={0.5} strokeDasharray="3,3"/>
                  <text x={xOf(d)+3} y={PAD_TOP-18} fontSize={9.5} fill="#888" fontFamily="system-ui" fontWeight="500">{dateLabel(d)}</text>
                  <text x={xOf(d)+3} y={PAD_TOP-7}  fontSize={8}   fill="#ccc" fontFamily="system-ui">d{d}</text>
                </g>
              ))}

              {Array.from({length:totalDays+1},(_,i)=>i).map(d=>(
                <line key={d} x1={xOf(d)} y1={SVG_H-24} x2={xOf(d)} y2={SVG_H-20} stroke="rgba(0,0,0,0.08)" strokeWidth={0.5}/>
              ))}
              ...
              {tasks.map(t=>{
                ...
                const tx=xOf(t.start),tw=Math.max(t.dur*DAY_W-3,4);
                ...
                return (
                  <g key={t.id}
                    ...
                    <rect x={tx} y={ty} width={tw} height={th} rx={5} fill={bg} opacity={isSel?1:0.85}
                    ...
                    {pctW>0&&<rect x={tx} y={ty+th-4} width={pctW} height={4} rx={0} fill={tc} opacity={0.35} style={{pointerEvents:"none"}}/>}
                    {tw>28&&(
                    ...
                    {isSel&&tw>20&&[0,2,4].map(off=>(
                      <line key={off} x1={tx+tw-RESIZE_HIT+1+off} y1={ty+6} x2={tx+tw-RESIZE_HIT+1+off} y2={ty+th-6}
              ...
              {milestones.map(m=>{
                ...
                return (
                  <g key={m.id}>
                    <line x1={mx} y1={PAD_TOP} x2={mx} y2={SVG_H-24} stroke="#185FA5" strokeWidth={1} strokeDasharray="4,3" opacity={0.3}/>
                    ...
                    <text x={mx} y={my+sz+12} textAnchor="middle" fontSize={9} fill="#185FA5" fontFamily="system-ui" opacity={0.65}>{dateLabel(m.day)}</text>
                  </g>
                );
              })}
              ...
              {todayOff>=0&&todayOff<=totalDays&&(
                <g>
                  <line x1={xOf(todayOff)} y1={PAD_TOP-16} x2={xOf(todayOff)} y2={SVG_H-24} stroke="#E24B4A" strokeWidth={1.5} opacity={0.7}/>
                  <rect x={xOf(todayOff)-18} y={PAD_TOP-30} width={36} height={14} rx={3} fill="#E24B4A"/>
                  <text x={xOf(todayOff)} y={PAD_TOP-19} textAnchor="middle" fontSize={8.5} fill="#fff" fontFamily="system-ui" fontWeight="600">TODAY</text>
                </g>
              )}
              ...
              <text x={SVG_W-10} y={SVG_H-9} textAnchor="end" fontSize={8.5} fill="#ddd" fontFamily="system-ui">
                {projectName} · {fmtDate(new Date(startDate))}
              </text>
            </svg>
          </div>
        </div>
        ...
        {selectedTask&&!presentMode&&(
          <div style={{width:256,background:"#fff",borderLeft:"0.5px solid rgba(0,0,0,0.08)",padding:18,overflowY:"auto",flexShrink:0}}>
            ...
            <div style={{background:"#f8f7f4",borderRadius:8,padding:"8px 10px",marginBottom:12,fontSize:12,color:"#555",lineHeight:1.6}}>
              <div>{dateLabel(selectedTask.start,false)}</div>
              <div style={{color:"#aaa",fontSize:11}}>→ {dateLabel(selectedTask.start+selectedTask.dur,false)} ({selectedTask.dur}d)</div>
            </div>
            <Field label="Owner">
            ...
            <button onClick={()=>deleteTask(selectedTask.id)}
              style={{width:"100%",fontSize:12,padding:"6px 0",marginTop:4,border:"0.5px solid rgba(220,50,50,0.25)",
            ...
            <div style={{marginTop:18,borderTop:"0.5px solid rgba(0,0,0,0.07)",paddingTop:14}}>
              ...
              <select onChange={e=>{if(!e.target.value)return;setDeps(ds=>[...ds,{from:e.target.value,to:selectedTask.id}]);e.target.value="";}} style={{...inputStyle,marginTop:4}}>
            ...
            <div style={{marginTop:18,borderTop:"0.5px solid rgba(0,0,0,0.07)",paddingTop:14}}>
              ...
              {milestones.map(m=>(
                <div key={m.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:5,alignItems:"center"}}>
                  <span style={{color:"#185FA5"}}>◆ {m.label} <span style={{color:"#aaa",fontSize:10}}>{dateLabel(m.day)}</span></span>
              ...
              }} style={{fontSize:12,padding:"4px 10px",border:"0.5px solid rgba(0,0,0,0.12)",borderRadius:6,background:"transparent",cursor:"pointer",fontFamily:"inherit",color:"#555"}}>
                + at task end
              </button>
            </div>
          </div>
      ...
      {tooltip&&(
        <div style={{position:"fixed",left:tooltip.x+14,top:tooltip.y-8,pointerEvents:"none",zIndex:300,
          ...
          <div style={{color:"#aaa"}}>{dateLabel(tooltip.task.start,false)} → {dateLabel(tooltip.task.start+tooltip.task.dur,false)}</div>
      ...
      {modal&&(
        <div onClick={()=>setModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.3)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:14,padding:28,width:540,maxHeight:"82vh",overflowY:"auto",boxShadow:"0 8px 40px rgba(0,0,0,0.12)"}}>

            {modal==="paste"&&(<>
              <ModalHeader title="Paste task data" onClose={()=>setModal(null)}/>
              <p style={{fontSize:12,color:"#999",marginBottom:12,lineHeight:1.7}}>
                CSV: <code style={codeStyle}>lane, label, start, dur, owner, tag, pct, rag</code><br/>
                Or JSON array. <code style={codeStyle}>rag</code>: G/A/R &nbsp;·&nbsp; <code style={codeStyle}>pct</code>: 0–100
              </p>
              <textarea value={pasteText} onChange={e=>setPasteText(e.target.value)}
                placeholder={"lane,label,start,dur,owner,tag,pct,rag\nEngineering,Design freeze,0,5,SYSO,Design,100,G\nRegulatory,Risk review,3,6,CLIN,Reg,40,A"}
                style={{width:"100%",height:160,fontFamily:"monospace",fontSize:12,padding:10,border:"0.5px solid rgba(0,0,0,0.15)",borderRadius:8,resize:"vertical",outline:"none"}}/>
            ...
            {modal==="templates"&&(<>
              ...
              <div style={{fontSize:12,color:"#999",marginBottom:12}}>Built-in</div>
              {BUILTIN_TEMPLATES.map(tpl=><TemplateRow key={tpl.id} name={tpl.name} onLoad={()=>loadTemplate(tpl)}/>)}
              {savedTemplates.length>0&&(<>
                <div style={{fontSize:12,color:"#999",margin:"16px 0 10px",borderTop:"0.5px solid rgba(0,0,0,0.07)",paddingTop:14}}>Saved</div>
              ...
              <div style={{borderTop:"0.5px solid rgba(0,0,0,0.07)",paddingTop:16,marginTop:16}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  <input value={tplName} onChange={e=>setTplName(e.target.value)} placeholder="Template name…"
                    onKeyDown={e=>e.key==="Enter"&&saveTemplate()} style={{...inputStyle,flex:1}}/>
            ...
            {modal==="owners"&&(<>
              ...
              <p style={{fontSize:12,color:"#999",marginBottom:16,lineHeight:1.6}}>Saved to your browser. Appear in owner dropdown when editing tasks.</p>
              <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:20}}>
                {owners.map(o=>(
                  <div key={o} style={{display:"flex",alignItems:"center",gap:5,background:"#f1efe8",borderRadius:20,padding:"4px 12px 4px 14px",fontSize:13,fontWeight:500}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <input value={newOwner} onChange={e=>setNewOwner(e.target.value.toUpperCase())}
                  placeholder="New team code…" maxLength={8} onKeyDown={e=>e.key==="Enter"&&addOwner()}
            ...
            {modal==="export"&&(<>
              <ModalHeader title="Export" onClose={()=>setModal(null)}/>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                <ExportOption icon="🖼" title="PNG image" desc="Best for PowerPoint, Teams, or email"
                  onClick={()=>{exportPNG();setModal(null);}}/>
                <ExportOption icon="📐" title="SVG vector" desc="Scalable — best for Figma or further editing"
                  onClick={()=>{exportSVG();setModal(null);}}/>
                <ExportOption icon="📄" title="PDF / Print" desc="Opens print dialog — choose Save as PDF"
                  onClick={()=>{exportPDF();setModal(null);}}/>
              </div>
            </>)}
...
function TemplateRow({name,onLoad,onDelete}) {
  return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,padding:"10px 14px",border:"0.5px solid rgba(0,0,0,0.08)",borderRadius:8,background:"#fafaf9"}}>
...
function ExportOption({icon,title,desc,onClick}) {
  return (
    <button onClick={onClick} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",border:"0.5px solid rgba(0,0,0,0.1)",borderRadius:10,background:"#fafaf9",cursor:"pointer",textAlign:"left",fontFamily:"inherit",width:"100%"}}>
      <span style={{fontSize:20}}>{icon}</span>
      <div>
        <div style={{fontSize:13,fontWeight:500,color:"#1a1a1a",marginBottom:2}}>{title}</div>
...
function Btn({children,onClick,accent,danger}) {
  return (
    <button onClick={onClick} style={{fontSize:12,padding:"6px 14px",fontFamily:"inherit",cursor:"pointer",borderRadius:7,
      border:danger?"0.5px solid rgba(220,50,50,0.25)":`0.5px solid ${accent?"#1a1a1a":"rgba(0,0,0,0.14)"}`,
      background:accent?"#1a1a1a":danger?"#fff8f8":"#fff",color:accent?"#fff":danger?"#A32D2D":"#333"}}>
      {children}
    </button>
  );
}
const inputStyle={width:"100%",fontSize:13,padding:"5px 8px",border:"0.5px solid rgba(0,0,0,0.14)",borderRadius:6,fontFamily:"inherit",outline:"none",background:"#fafafa",color:"#1a1a1a"};
const codeStyle={background:"#f5f5f3",padding:"1px 5px",borderRadius:4,fontSize:11,fontFamily:"monospace"};